...
class BaseModelMixin(object):
    ...
    def mmx_similarity_matrix(self, labels, **kwargs):
        ...
        # label similarity matrix
        M = LabelMatrix(rows=labels, columns=labels)
        ...
        for i, label in enumerate(labels):
            for j, other_label in enumerate(labels):
                ...
                # compute similarity
                M[label, other_label] = self.mmx_compare(
                    label, other_label, models=models, **kwargs)

                # if similarity is symmetric, propagate Xij to Xji
                if self.mmx_symmetric():
                    M[other_label, label] = M[label, other_label]
...
class PrecomputedMMx(BaseModelMixin):
    ...
    def mmx_similarity_matrix(self, labels, feature=None, **kwargs):
        ...
        return matrix.subset(rows=set(labels), columns=set(labels))
...
class AverageLinkMMx(PrecomputedMMx):
    ...
    def mmx_compare(self, label, other_label, models=None, **kwargs):
        ...
        return np.mean(
            self.feature.subset(
                rows=set(model), columns=set(other_model)
            ).df.values
        )
